...
export default function LoginPage() {
  ...
  const demoAccounts = [
    ...
    { email: 'verifier@client.com', role: 'Client Verifier', color: 'bg-green-500' },
    { email: 'authorizer@client.com', role: 'Client Authorizer', color: 'bg-purple-500' },
    { email: 'processor@gis.com', role: 'GIS Processor', color: 'bg-orange-500' },
  ...
  return (
    <div className="w-full">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center justify-center w-16 h-16 bg-[#D92027] rounded-xl mb-4 animate-pulse">
          <Building2 className="w-8 h-8 text-white" />
        </div>
        <h1 className="text-2xl font-bold text-white mb-1">CIS Portal</h1>
        <p className="text-white/50 text-sm">Client Instruction Submission - UBA</p>
      </div>
      ...
      <div className="bg-[#1A1A1A]/80 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-2xl">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <button type="button" className="text-sm text-[#D92027] hover:underline">
              Forgot password?
            </button>
          </div>
        ...
        <div className="mt-6 pt-6 border-t border-white/10">
          <p className="text-xs text-white/40 text-center mb-4">Demo Accounts (Password: "password")</p>
          <div className="grid grid-cols-1 gap-2">
            {demoAccounts.map((account) => (
              ...
              >
                <div className={cn("w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0", account.color)}>
                  <span className="text-xs font-bold text-white">{account.role.charAt(0)}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-white truncate">{account.role}</p>
